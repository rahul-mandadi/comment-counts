...
`machine_labels.json` holds 100 pairs labelled by an LLM judge (claude-opus-5) against the
...
`for_human.jsonl` holds 25 pairs, 5 per similarity band (the five bands at 0.80 and above, which is where every candidate threshold sits), for Rahul to label independently.
